...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        # 使用 tqdm 进度条，显示当前 epoch 的批处理进度
        progress_bar = tqdm(enumerate(train_data), total=len(train_data), desc=f"Epoch {epoch_idx}", leave=False)

        for batch_idx, interaction in progress_bar:
            ...
            # 更新进度条的显示，显示当前批次的平均损失
            avg_loss = total_loss / (batch_idx + 1) if batch_idx > 0 else total_loss
