...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            brand_path=os.path.abspath(config['data_path']+'brand'+config['dataset']+'image_feat.npy')
            # if file exist?
            ...
            if os.path.isfile(brand_path):
                self.b_feat = torch.from_numpy(np.load(brand_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
